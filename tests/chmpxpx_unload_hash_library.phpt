...
if(file_exists("/usr/lib/libk2htesthash.so") && !chmpxpx_load_hash_library("/usr/lib/libk2htesthash.so")){
    echo "NG";
    exit;
}else if(file_exists("/usr/lib/x86_64-linux-gnu/libk2htesthash.so") && !chmpxpx_load_hash_library("/usr/lib/x86_64-linux-gnu/libk2htesthash.so")){
    echo "NG";
    exit;
}
if(chmpxpx_unload_hash_library()){
    echo "OK";
}else{
    echo "NG";
}
