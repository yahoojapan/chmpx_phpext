        ...
        if(false !== $svrobj && is_object($svrobj)){
            ...
            if($svrobj->ReceiveKV($rcvkey, $rcvval, 3000, true)){
                if($rcvkey == $sndkey && $rcvval == $sndval){
                    ...
                    if($svrobj->ReplyKV($replykey, $replyval)){
                        // OK
                    }else{
                        echo "NG: Failed replying key and value.\n";
                    }
                }else{
                    echo "NG: Received data is wrong.(" . $sndkey . " -> " . $rcvkey . ", " . $sndval . " -> " . $rcvval . ")\n";
                }
            }else{
    ...
    if(isset($slvprocid) && $chmpx->CreateSlave("./tests/slave.yaml", true)){
        ...
        if(false !== $slaveobj && is_object($slaveobj)){
            ...
            if(1 <= $count){
                ...
                $rcvkey		= "";
                $rcvval		= "";
                if($slaveobj->ReceiveKV($rcvkey, $rcvval)){
                    if($replykey == $rcvkey && $replyval == $rcvval){
                        echo "OK";
                    }else{
                        echo "NG: Received data is wrong(replykey->" . $replykey . ", rcvkey->" . $rcvkey . ", replyval->" . $replyval . ", rcvval->" . $rcvval . ").";
                    }
                }else{
