...
$chmpx = new Chmpx();
if($chmpx->CreateSlave("./tests/slave.yaml", true)){
    $slaveobj = $chmpx->GetChmpxSlave();
    if(false !== $slaveobj){
        if(is_object($slaveobj)){
            echo "OK";
        }else{
            echo "NG: Could not get chmpx slave object.";
        }
        unset($slaveobj);
    }else{
        echo "NG: Failed to get chmpx slave object.";
    }
    ...
    echo "NG: Could not create chmpx object.";
}
unset($chmpx);
